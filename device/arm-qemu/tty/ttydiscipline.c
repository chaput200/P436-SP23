...
int ttydiscipline(char ch,
  ...
  if (ch == TY_NEWLINE || ch == TY_RETURN) {
     ...
     */
	  for (int i=0; i < typtr->tyicursor; i++) {
		  typtr->typrev[i] = typtr->tyibuff[i];
	  }
	  return DO_PRINT;
  ...
  if(typtr->tycommand == TY_BRACE && ch == TY_A){
	  //PERFORM UP ARROW BEHAVIOR
	  clearline(typtr,csrptr);
	  printf("typtr->tyicursor = %d\n", typtr->tyicursor);
	  for (int i=0; i < TY_IBUFLEN; i++) {
		  typtr->tyibuff[i] = typtr->typrev[i];
	  }
	  printf("typtr->tyibuff[0] = %c\n", typtr->tyibuff[0]);
	  printf("typtr->tyicursor = %d\n", typtr->tyicursor);
	  for (int i=0; i < typtr->tyicursor; i++) {
		  //echo(typtr->tyibuff[i], typtr, csrptr);
	  }
	  return SKIP;
  }
